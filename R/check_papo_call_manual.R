...
check_papo_call <- function(datasets, module_id, subject_level_dataset_name, subjid_var,
  ...
  if (!missing(plots) && !is.null(plots)) {
    ...
    if (is.character(plots)) {
      assert_err(
        checkmate::test_character(plots, pattern = "^defaults$", len = 1),
        "`plots` must be a named list (see vignettes) or a character string with value 'defaults' to use default values for the plots."
      )
    } else {
