...
Runs the server that populates the UI returned by
...
- subject_level_dataset:
  ...
  `[reactive(data.frame)]` Visit-independent subject information.

- extra_datasets:

  `[reactive(data.frame(n))]` Visit-dependent subject datasets.
